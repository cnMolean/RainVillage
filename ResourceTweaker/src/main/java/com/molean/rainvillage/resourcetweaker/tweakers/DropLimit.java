...
public class DropLimit implements Listener {
    ...
    private boolean canObtain(long chunkID, ItemStack itemStack) {
    ...
    public void on(EntityDropItemEvent event) {
        ...
        Chunk chunk = event.getItemDrop().getChunk();
        ItemStack itemStack = event.getItemDrop().getItemStack();
        if (!canObtain(chunk.getChunkKey(), itemStack)) {
    ...
    public void on(BlockDropItemEvent event) {
        ...
        for (Item item : items) {
            if (!canObtain(chunk.getChunkKey(), item.getItemStack())) {
                tobeDelete.add(item);
            }
        }
